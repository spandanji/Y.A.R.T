...
uint64_t segmented_seive(const uint64_t& n)
{
    uint64_t sqrt_n = (uint64_t)std::sqrt(n);
    const uint64_t seive_size = std::max(sqrt_n, (uint64_t)32768);
    ...
    for(uint64_t lower_limit = 0; lower_limit <= n; lower_limit += seive_size)
    {
        ...
        {
            if(is_prime[is_prime_i])
                for(uint64_t j = is_prime_i * is_prime_i; j <= sqrt_n; j += is_prime_i)
                    is_prime[j] = 0;
            is_prime_i += 2;
        }
